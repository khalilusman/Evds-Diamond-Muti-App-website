import { ReactNode, useState } from 'react'
import { NavLink, useNavigate } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import Logo from '../components/Logo'
import { DiamondDiscIcon, SerialScanIcon, JobRecordIcon, SATIncidentIcon, BridgeCutterIcon } from '../components/icons/EvdsIcons'
import ThemeToggle from '../components/ThemeToggle'
import LanguageSwitcher from '../components/LanguageSwitcher'
import useAuthStore from '../stores/auth.store'

interface AppLayoutProps {
  children: ReactNode
}

type NavItem = {
  to: string
  labelKey: string
  icon: React.ReactNode
}

const MY_DISCS: NavItem = {
  to: '/my-discs',
  labelKey: 'nav.my_discs',
  icon: <DiamondDiscIcon size={22} stroke="currentColor" />,
}

const ACTIVATE: NavItem = {
  to: '/activate',
  labelKey: 'nav.activate',
  icon: <SerialScanIcon size={22} stroke="currentColor" />,
}

const USAGE: NavItem = {
  to: '/usage',
  labelKey: 'nav.usage',
  icon: <JobRecordIcon size={22} stroke="currentColor" />,
}

const STATS: NavItem = {
  to: '/stats',
  labelKey: 'nav.stats',
  icon: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
    </svg>
  ),
}

const SAT: NavItem = {
  to: '/sat',
  labelKey: 'nav.sat',
  icon: <SATIncidentIcon size={22} stroke="currentColor" />,
}

const COST: NavItem = {
  to: '/cost',
  labelKey: 'nav.cost',
  icon: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
    </svg>
  ),
}

const MACHINES: NavItem = {
  to: '/machines',
  labelKey: 'nav.machines',
  icon: <BridgeCutterIcon size={22} stroke="currentColor" />,
}

const PROFILE: NavItem = {
  to: '/profile',
  labelKey: 'nav.profile',
  icon: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
    </svg>
  ),
}

export default function AppLayout({ children }: AppLayoutProps) {
  const { t } = useTranslation()
  const { user, logout } = useAuthStore()
  const navigate = useNavigate()
  const isAdmin = user?.role === 'CUSTOMER_ADMIN'

  function handleLogout() {
    logout()
    navigate('/login')
  }

  const activeClass =
    'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
  const inactiveClass =
    'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'

  const [moreOpen, setMoreOpen] = useState(false)

  const moreNavItems: NavItem[] = [
    ...(isAdmin ? [MACHINES] : []),
    SAT,
    STATS,
    PROFILE,
  ]

  const sideNavItems: NavItem[] = [
    MY_DISCS,
    ...(isAdmin ? [ACTIVATE] : []),
    USAGE,
    STATS,
    SAT,
    COST,
    ...(isAdmin ? [MACHINES] : []),
    PROFILE,
  ]

  const mobileNavItems: NavItem[] = [
    MY_DISCS,
    ...(isAdmin ? [ACTIVATE] : []),
    COST,
    USAGE,
  ]

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      {/* Header */}
      <header className="sticky top-0 z-50 h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Logo size="sm" />
          <span className="font-semibold text-gray-900 dark:text-white hidden sm:block">
            EVDS Nexus
          </span>
        </div>

        <div className="flex items-center gap-2">
          <ThemeToggle />
          <LanguageSwitcher />
          <span className="hidden sm:block text-sm text-gray-600 dark:text-gray-400 ml-2">
            {user?.name}
          </span>
          <button
            type="button"
            onClick={handleLogout}
            className="text-sm text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors ml-1"
          >
            {t('common.logout')}
          </button>
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar — desktop only */}
        <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800">
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            {sideNavItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                    isActive ? activeClass : inactiveClass
                  }`
                }
              >
                {item.icon}
                {t(item.labelKey)}
              </NavLink>
            ))}
          </nav>
        </aside>

        {/* Main content */}
        <main className="flex-1 overflow-auto p-4 sm:p-6 pb-24 md:pb-6">
          {children}
        </main>
      </div>

      {/* Bottom nav — mobile only */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex z-40">
        {mobileNavItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            className={({ isActive }) =>
              `flex-1 flex flex-col items-center justify-center py-2 gap-1 transition-colors ${
                isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-500'
              }`
            }
          >
            {item.icon}
            <span className="text-xs font-medium">{t(item.labelKey)}</span>
          </NavLink>
        ))}
        <button
          type="button"
          onClick={() => setMoreOpen(true)}
          className="flex-1 flex flex-col items-center justify-center py-2 gap-1 text-gray-500 dark:text-gray-500"
        >
          <svg className="w-[22px] h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z" />
          </svg>
          <span className="text-xs font-medium">More</span>
        </button>
      </nav>

      {/* More drawer */}
      {moreOpen && (
        <>
          <div
            className="md:hidden fixed inset-0 z-50 bg-black/40"
            onClick={() => setMoreOpen(false)}
          />
          <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 rounded-t-2xl border-t border-gray-200 dark:border-gray-800">
            <div className="p-4 space-y-1">
              <div className="w-10 h-1 bg-gray-300 dark:bg-gray-700 rounded-full mx-auto mb-4" />
              {moreNavItems.map((item) => (
                <button
                  key={item.to}
                  type="button"
                  onClick={() => { navigate(item.to); setMoreOpen(false) }}
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                >
                  {item.icon}
                  {t(item.labelKey)}
                </button>
              ))}
            </div>
          </div>
        </>
      )}
    </div>
  )
}
