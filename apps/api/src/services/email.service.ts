...
export async function sendRegistrationConfirm(to: string, companyName: string): Promise<void> {
  await send(
    to,
    'Registration received — EVDS Diamond',
    `<p>Hello,</p>
     <p>Thank you for registering <strong>${companyName}</strong> on EVDS Diamond.</p>
...
export async function sendAccountApproved(to: string, companyName: string): Promise<void> {
  await send(
    to,
    'Account approved — EVDS Diamond',
    `<p>Hello,</p>
     <p>Great news! Your account for <strong>${companyName}</strong> has been approved.</p>
     <p>You can now log in and start using EVDS Diamond:</p>
...
export async function sendAccountSuspended(
  ...
  await send(
    to,
    'Account suspended — EVDS Diamond',
    `<p>Hello,</p>
...
export async function sendAccountDeactivated(
  ...
  await send(
    to,
    'Account deactivated — EVDS Diamond',
    `<p>Hello,</p>
...
export async function sendPasswordReset(to: string, resetLink: string): Promise<void> {
  await send(
    to,
    'Reset your password — EVDS Diamond',
    `<p>Hello,</p>
     <p>You requested a password reset for your EVDS Diamond account.</p>
...
export async function sendSatTicketResolved(
  ...
  await send(
    to,
    'Support ticket resolved — EVDS Diamond',
    `<p>Hello,</p>
