...
const RESULT_LABELS: Record<string, string> = {
  ...
  max_activations_reached:  'Max Reached',
  voided:                   'Voided',
  wrong_combination:        'Wrong Combo',
}
...
function VoidModal({ alert, onClose, onDone }: VoidModalProps) {
  ...
  const mut = useMutation({
    mutationFn: () => {
      if (!reason.trim()) { setError('Reason is required'); throw new Error('empty') }
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-sm space-y-4">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">
          Void code <span className="font-mono text-red-600">{alert.unique_code}</span>?
        </h2>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          This will permanently void the code and prevent future activation attempts.
        </p>
        <div className="space-y-1">
          ...
          <textarea
            ...
            rows={3}
            placeholder="e.g. Suspected fraud, duplicate scan..."
        ...
        <div className="flex gap-3">
          ...
          <Button variant="danger" fullWidth loading={mut.isPending} onClick={() => mut.mutate()}>Void Code</Button>
        </div>
      </div>
    </div>
  )
    ...
    <DashboardLayout title="Security Alerts">
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: 'Failed Attempts (today)', value: failedToday, color: 'border-orange-500' },
            { label: 'Suspicious Codes', value: suspicious, color: 'border-red-500' },
            { label: 'Voided Codes', value: voided, color: 'border-gray-400' },
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow overflow-hidden">
          {isLoading ? (
            ...
            <div className="py-16 text-center text-sm text-gray-400 dark:text-gray-500">No security alerts found</div>
          ) : (
            <>
